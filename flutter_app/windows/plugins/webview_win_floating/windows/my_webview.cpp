...
HRESULT MyWebViewImpl::setCookie(LPCWSTR domain, LPCWSTR name, LPCWSTR value, LPCWSTR path)
{
    wil::com_ptr<ICoreWebView2CookieManager> cookieManager;
    auto webview2_2 = m_pWebview.try_query<ICoreWebView2_2>();
    if (webview2_2 == NULL) return E_FAIL;
    ...
    auto hr = cookieManager->CreateCookie(name, value, domain, path, &cookie);
